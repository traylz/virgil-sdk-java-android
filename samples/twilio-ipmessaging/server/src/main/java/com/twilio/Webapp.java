...
public class Webapp {
	...
	public static void main(String[] args) {
		...
		// Create a Faker instance to generate a random username for the
		// connecting user
		...
		get("/token", "application/json", (request, response) -> {
			...
			if (identity == null || identity.trim().isEmpty()) {
				identity = faker.firstName() + faker.lastName() + faker.zipCode();
			}

			// Create an endpoint ID which uniquely identifies the user on their
			// current device
			String appName = "TwilioChatDemo";
			String endpointId = appName + ":" + identity + ":" + request.queryParams("device");

			System.out.println("New endpoint: " + endpointId);
			...
			AccessToken token = new AccessToken.Builder(System.getenv("TWILIO_ACCOUNT_SID"),
					System.getenv("TWILIO_API_KEY"), System.getenv("TWILIO_API_SECRET")).identity(identity).grant(grant)
							.build();
