...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (prefs.contains(Constants.Prefs.IDENTITY)) {
            // Open main activity if user is already registered
            openChat();
            return;
        }

        // Generate key pair
        mKeyPair = KeyPairGenerator.generate();
    }
    ...
    private void attemptLogin() {
        ...
        if (cancel) {
            ...
            showProgress(true);
            mAuthTask = new UserRegisterTask(email);
    ...
    public class UserRegisterTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            ClientFactory clientFactory = new ClientFactory(accessToken);

            VirgilCardTemplate.Builder builder = new VirgilCardTemplate.Builder();
            builder.setIdentity(new ValidatedIdentity(IdentityType.EMAIL, mEmail));
            builder.setPublicKey(mKeyPair.getPublic());

            try {
                String actionId = clientFactory.getIdentityClient().verify(IdentityType.EMAIL, mEmail);


                VirgilCard card = clientFactory.getPublicKeyClient().createCard(builder.build(), mKeyPair.getPrivate());

                mCardId = card.getId();

                Log.d(TAG, "Registered Identity '" + mEmail + "', Card ID: " + card.getId());
            }
        ...
        protected void onPostExecute(final Boolean success) {
            ...
            if (success) {
                SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(getBaseContext());
                prefs.edit()
                        .putString(Constants.Prefs.IDENTITY, mEmail)
                        .putString(Constants.Prefs.CARD_ID, mCardId)
                        .putString(Constants.Prefs.PRIVATE_KEY, mKeyPair.getPrivate().getAsString())
                        .commit();
                openChat();
            } else {
